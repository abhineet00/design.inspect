...
${body}
  ...
  function subHead(label, onAdd, { disabled = false } = {}) {
    return h("div", { class: "sub-head" }, [
      h("span", { class: "sub-label", text: label }),
      h("button", {
        class: "sub-add" + (disabled ? " off" : ""),
        title: "Add " + label,
        html: icon("plus"),
  ...
  function linkToggle(linked, onToggle) {
    return h("button", {
  ...
  function colorRow(value, onChange, onRemove) {
  ...
  var Panel = class {
    ...
    _design(body) {
      ...
      const onW = (v) => {
        ...
        if (linked && ratio && isFinite(n)) {
          setProp(el, "height", Math.round(n / ratio) + "px");
          this.render();
        }
      };
      const onH = (v) => {
        ...
        const n = parseFloat(v);
        if (linked && ratio && isFinite(n)) {
          setProp(el, "width", Math.round(n * ratio) + "px");
          this.render();
        }
      };
      ...
      if (cornerExp) {
        const corners = [
          ["border-top-left-radius", m.radius.tl],
          ["border-top-right-radius", m.radius.tr],
      ...
      if (strokeOn) {
        app.push(colorRow(
          ...
          on("border-color"),
          () => {
            setProp(el, "border-width", "0px");
            this.render();
          }
        ));
        const sw = m.border.sides;
        const widthMixed = (/* @__PURE__ */ new Set([sw.top, sw.right, sw.bottom, sw.left])).size > 1;
        ...
        app.push(h("div", { class: "corner-mix" }, [
          ...
          expandBtn(strokeExp, "border-all-01", strokeExp ? "Collapse sides" : "Edit each side", () => {
            this._strokeSidesExpanded = !strokeExp;
            this.render();
          })
        ]));
        if (strokeExp) {
          const sides2 = [
            ["border-top-width", "stroke-top", sw.top],
            ["border-right-width", "stroke-right", sw.right],
      ...
      body.append(section("Typography", [
        ...
        colorRow(m.typography.color, on("color"), () => {
          setProp(el, "color", "inherit");
          this.render();
        })
      ]));
    }
    ...
    _html(body) {
      this._expanded = this._expanded || /* @__PURE__ */ new Set();
    ...
    _renderNode(el, container, depth) {
      ...
      const tw = h("span", {
        class: "tree-tw" + (hasKids ? "" : " leaf"),
        html: hasKids ? expanded ? TREE_ICONS.caretDown : TREE_ICONS.caretRight : ""
      });
      tw.addEventListener("click", (e) => {
        e.stopPropagation();
        if (!hasKids) return;
        if (expanded) this._expanded.delete(el);
        else this._expanded.add(el);
        this.render();
      });
      const row = h("div", {
        class: "tree-row" + (el === this.selected ? " selected" : ""),
      ...
      if (hasKids && expanded) {
        ...
        container.append(h(
          "div",
          { class: "tree-row tree-close", style: { paddingLeft: pad + "px" } },
...
font-weight: ${t.weight};`, "Type style copied")
  ...
  function tagOpenHtml(el, showEllipsis) {
    ...
    for (const a of el.attributes) {
      ...
      const val2 = a.value.length > 40 ? a.value.slice(0, 40) + "\u2026" : a.value;
      attrs += ` <span class="t-attr">${escapeHtml2(a.name)}</span><span class="t-br">=</span><span class="t-val">"${escapeHtml2(val2)}"</span>`;
    }
    ...
    constructor() {
      ...
      this.label = h("div", {
        ...
        style: {
          ...
          background: "rgba(0,0,0,0.82)",
          color: "#fff",
          font: "500 12px 'Quicksand', -apple-system, sans-serif",
          padding: "4px 10px",
          borderRadius: "8px",
